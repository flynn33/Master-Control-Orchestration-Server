...
Phase file: [handoff/realignment/PHASE-09-tron-dashboard-realignment.md](PHASE-09-tron-dashboard-realignment.md)
...
| [docs/implementation/DASHBOARD-ROUTE-MAP.md](../../docs/implementation/DASHBOARD-ROUTE-MAP.md) | New canonical reference mapping each panel to its feeding routes. Documents the honest-telemetry rule and lists the surface-compliance hard rules. |
| [docs/implementation/ARCHITECTURE-DRIFT-INVENTORY.md](../../docs/implementation/ARCHITECTURE-DRIFT-INVENTORY.md) | Section G expanded from one drift row to eleven "done" rows covering gateway / pools / clients / governance / onboarding / discovery / activity / honesty / manual-setup. WinUI shell row remains the single deferred-cleanup item. |
| [docs/implementation/FORBIDDEN-CONTRACT-GREP-LIST.md](../../docs/implementation/FORBIDDEN-CONTRACT-GREP-LIST.md) | New Group 8 — Dashboard honesty: §8.1 forbids `.toFixed(...)` on heartbeat / worker-telemetry sites; §8.2 forbids legacy hardcoded surface IDs in index.html; §8.3 forbids provider-era residue in app.js (mirrors Forsetti script); §8.4 forbids reintroduced `/api/providers` fetches. |
...
2. **MCPJungle-backed gateway not exercised end-to-end.** Pools panel renders correctly when `/api/pools` is empty (the supervised-mock fallback). Per-pool fetches degrade silently when a pool has no instances. A real MCPJungle binary integration test is deferred to PHASE-10.
...
| MCPJungle-backed end-to-end gateway / pool exercise | PHASE-10 | Requires a real spawnable MCPJungle binary; release-gate concern. |
...
3. Exercising the MCPJungle-backed gateway end-to-end (the static-only validation of PHASE-02..09 becomes runtime validation here).
