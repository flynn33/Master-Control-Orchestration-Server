...
Phase file: [handoff/realignment/PHASE-10-windows-hardening-ci-release.md](PHASE-10-windows-hardening-ci-release.md)
...
| [.github/workflows/release.yml](../../.github/workflows/release.yml) | New workflow. Triggers on `v*` tag only. Validates VERSION.json matches tag. Queries GitHub API for the same-SHA `windows-build-test-package.yml` run and demands `conclusion=success`. Downloads gating artifact, validates contents, publishes release. |
| [docs/wiki/Operations/Windows-Firewall-LAN-Mode.md](../../docs/wiki/Operations/Windows-Firewall-LAN-Mode.md) | New. Trust model boundary, 4 `New-NetFirewallRule` snippets, Public-profile blocks, validation snippets, uninstall cleanup. |
| [docs/wiki/Operations/Packaging-and-Gateway-Binary.md](../../docs/wiki/Operations/Packaging-and-Gateway-Binary.md) | New. What MSI installs, why MCPJungle is operator-installed (ADR-002 §11), supervised-mock fallback, post-install smoke (`bootstrapper preflight`, console mode), uninstall behavior. |
| [docs/wiki/Operations/Release-Gate.md](../../docs/wiki/Operations/Release-Gate.md) | New. Workflow pair, why no `workflow_dispatch`, tag → release flow, toolchain hardening, version-stamping order, failure-mode triage. |
...
| [docs/implementation/FORBIDDEN-CONTRACT-GREP-LIST.md](../../docs/implementation/FORBIDDEN-CONTRACT-GREP-LIST.md) | Group 6 expanded: §6.2 forbid YAML `workflow_dispatch:` trigger on gating workflows, §6.3 forbid hardcoded `\Enterprise\` / `/Enterprise/` path segments in workflow files, §6.4 process-execution 7-step compliance with documented bootstrapper exemptions, §6.5 awk-based ordering check that the version-stamp step precedes configure. |
| [docs/implementation/ARCHITECTURE-DRIFT-INVENTORY.md](../../docs/implementation/ARCHITECTURE-DRIFT-INVENTORY.md) | Section I (CI / release / packaging) expanded from 4 drift rows to 9 "done" rows. The bootstrapper-rewrite item is the single deferred entry. |
...
| `scripts/check-mastercontrol-forsetti.ps1` | **PASS** — `Master Control Forsetti checks passed.` | Vendoring untouched; new files all under `docs/wiki/Operations/`. |
...
| Install → use → uninstall round-trip smoke on a clean VM | Future hardening track | Requires a Windows Sandbox / fresh-VM runner. Documented in `docs/wiki/Operations/Packaging-and-Gateway-Binary.md` as the gold-standard smoke. |
...
| MCPJungle-backed end-to-end gateway exercise | Future hardening track | Requires a real MCPJungle binary. Documented in the Packaging doc as an operator concern; the supervised-mock fallback honors ADR-002 §9 in the absence of a binary. |
