...
- **Forbidden patterns** — section 2.1 of `FORBIDDEN-CONTRACT-GREP-LIST.md` now permits the substrings `native HTTP.sys gateway`/`native HTTP.sys gateway`/`native HTTP.sys gateway` to appear inside the adapter file pair, the enum string tables, and the legacy `GatewayType` slot declaration; anywhere else is a coupling regression.
