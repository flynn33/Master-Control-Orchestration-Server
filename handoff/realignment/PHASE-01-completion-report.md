...
1. Reading [handoff/realignment/PHASE-02-mcp-gateway-spike-native HTTP.sys gateway.md](PHASE-02-mcp-gateway-spike-native HTTP.sys gateway.md) and its `readFirst` files (`src/MasterControlApp/MasterControlRuntime.cpp`, `src/MasterControlServiceHost`, `src/MasterControlModules`, `CMakeLists.txt`, `docs/implementation/MCOS-REALIGNMENT-MASTER.md`).
