...
- Supersedes: the v0.6.7 honest-503 listener (`NativeHttpSysGatewayAdapter::startHonestUnavailableListenerLocked`) for the in-process portion. the in-process HTTP.sys adapter remains supported via the existing `NativeHttpSysGatewayAdapter` for operators who prefer the supervised-binary path.
...
ADR-003 made the in-process HTTP.sys adapter the v0.6.x default and reserved this phase for the rebuild. Operator-experience-friction trigger #4 has fired ("New operators consistently fail to install the in-process HTTP.sys adapter alongside MCOS"). Building native eliminates the friction completely and aligns the substrate with `.claude/rules/10-windows-native-cpp.md`'s "Windows-native first" rule.
...
- The native HTTP.sys adapter remains shipped and selectable. Operators who already have the in-process HTTP.sys adapter running get to keep it. `mcpGateway.type` switches between `"native"` (existing adapter) and `"native"` (new adapter).
...
9. **Gateway selection at construction**: MCOS reads `mcpGateway.type`; constructs `NativeHttpSysGatewayAdapter` when `"native"`, falls back to `NativeHttpSysGatewayAdapter` for `"native"` (default for backward compat).
...
| `docs/wiki/Gateway.md` | Edit | New "native" section explaining the trade-off matrix vs the in-process HTTP.sys adapter. |
...
3. `LeaseRouter` selection and sticky-session contract continue to behave identically to the in-process HTTP.sys adapter path. PHASE-07 sticky-session test pins this.
4. Switching `mcpGateway.type` between `"native"` and `"native"` via POST `/api/config` and a service restart fully reconfigures the substrate without code change.
...
6. `MasterControlOrchestrationServer-vX.Y.Z-win-x64.msi` ships with the native gateway selected by default. Operators who want the in-process HTTP.sys adapter can flip the config field.
...
- The MVP is enough to retire the legacy external-gateway dependency for new installs. Existing the in-process HTTP.sys adapter deployments keep working via the preserved adapter.
...
| Operator confusion about `mcpGateway.type`. | Default stays `"native"` until PHASE-12 ships v1.0; flip default to `"native"` in v0.7.0. |
