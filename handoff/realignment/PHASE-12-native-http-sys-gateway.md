...
- Supersedes: the v0.6.7 honest-503 listener (`McpJungleGatewayAdapter::startHonestUnavailableListenerLocked`) for the in-process portion. MCPJungle remains supported via the existing `McpJungleGatewayAdapter` for operators who prefer the supervised-binary path.

## Mission

Replace the supervised MCPJungle child process with a Windows-native MCP Gateway implemented inside MCOS in C++ on top of HTTP.sys. After PHASE-12, operators get a usable gateway out of the box with no external binary to install.

## Why

ADR-003 made MCPJungle the v0.6.x default and reserved this phase for the rebuild. Operator-experience-friction trigger #4 has fired ("New operators consistently fail to install MCPJungle alongside MCOS"). Building native eliminates the friction completely and aligns the substrate with `.claude/rules/10-windows-native-cpp.md`'s "Windows-native first" rule.
...
- The MCPJungle adapter remains shipped and selectable. Operators who already have MCPJungle running get to keep it. `mcpGateway.type` switches between `"mcpjungle"` (existing adapter) and `"native"` (new adapter).
...
1. **`NativeHttpSysGatewayAdapter` class** implementing `IMcpGateway`. Drop-in alongside `McpJungleGatewayAdapter`.
...
9. **Gateway selection at construction**: MCOS reads `mcpGateway.type`; constructs `NativeHttpSysGatewayAdapter` when `"native"`, falls back to `McpJungleGatewayAdapter` for `"mcpjungle"` (default for backward compat).
...
| `src/MasterControlApp/MasterControlRuntime.cpp` | Edit | Adapter-selection logic in the runtime construction site (currently hardcodes `McpJungleGatewayAdapter`). |
...
| `docs/wiki/Gateway.md` | Edit | New "Native HTTP.sys gateway" section explaining the trade-off matrix vs MCPJungle. |
...
3. `LeaseRouter` selection and sticky-session contract continue to behave identically to the MCPJungle path. PHASE-07 sticky-session test pins this.
4. Switching `mcpGateway.type` between `"native"` and `"mcpjungle"` via POST `/api/config` and a service restart fully reconfigures the substrate without code change.
...
6. `MasterControlOrchestrationServer-vX.Y.Z-win-x64.msi` ships with the native gateway selected by default. Operators who want MCPJungle can flip the config field.
...
- The MVP is enough to retire the MCPJungle dependency for new installs. Existing MCPJungle deployments keep working via the preserved adapter.
...
| Operator confusion about `mcpGateway.type`. | Default stays `"mcpjungle"` until PHASE-12 ships v1.0; flip default to `"native"` in v0.7.0. |
...
- PHASE-07 lease router contract — unchanged. Native gateway calls into the existing `LeaseRouter` exactly the same way the MCPJungle adapter would have.
