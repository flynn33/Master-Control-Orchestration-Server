...
Phase file: [handoff/realignment/PHASE-03-bonjour-lan-discovery.md](PHASE-03-bonjour-lan-discovery.md)
...
| [docs/implementation/ARCHITECTURE-DRIFT-INVENTORY.md](../../docs/implementation/ARCHITECTURE-DRIFT-INVENTORY.md) | Section A (LAN discovery surface) rows flipped from "extend"/"replace (new)" to "done" with implementation paths. |
| [docs/implementation/FORBIDDEN-CONTRACT-GREP-LIST.md](../../docs/implementation/FORBIDDEN-CONTRACT-GREP-LIST.md) | New section 1.7a (discovery document `auth`/`trust` integrity) — regression detection for any attempt to weaken the const `auth=none` / `trust=lan` invariants on the AI-client surface. |
...
1. Reading [handoff/realignment/PHASE-04-model-specific-onboarding-profiles.md](PHASE-04-model-specific-onboarding-profiles.md) and its `readFirst` files (`resources/web/app.js`, `docs/wiki/Client-Config-Bundle.md`, `src/MasterControlApp/MasterControlRuntime.cpp`, `.mcp.json`).
