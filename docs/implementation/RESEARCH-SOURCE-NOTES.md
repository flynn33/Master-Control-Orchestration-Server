...
- Native HTTP.sys adapter is the current and only shipping gateway substrate (v0.9.0+). the legacy external gateway was retired in the same release. The substrate-agnostic `IMcpGateway` interface preserved the option to swap later, which made the v0.9.0 retirement a clean delete.
