...
This file maps where the current MCOS implementation conflicts with the gateway-first model declared in [ADR-002](../wiki/ADR-002-gateway-first-mcp-realignment.md). It is a forward-looking inventory: each row identifies an existing surface and the phase that resolves it. PHASE-00 makes no edits; this is the map subsequent phases consume.
...
| Action | `keep` (no change), `extend` (add to it), `subsume` (replace with new endpoint and deprecate), `replace` (delete and rebuild), `split` (maintainer vs AI-client paths). |
...
| `instanceId` persistence | **Landed PHASE-03.** `AppConfiguration::instanceId` field added; `buildDefaultConfiguration()` generates a UUID-backed identifier (`mcos-<uuid>`) via Win32 `UuidCreate`. Maintainers can override by editing `mcos.json`; the field round-trips through every config persistence path. | Generated. | PHASE-03 | done |

## B. MCP Gateway surface (PHASE-02 resolution)

| Surface | Today | Realignment target | Resolves in | Action |
|---|---|---|---|---|
| MCP Gateway adapter | **Landed PHASE-02.** `IMcpGateway` lives at `include/MasterControl/MasterControlContracts.h`. `NativeHttpSysGatewayAdapter` (production) and `FakeMcpGatewayAdapter` (tests) live at `include/MasterControl/McpGatewayAdapters.h` and `src/MasterControlApp/McpGatewayAdapters.cpp`. Adapter is replaceable behind the interface. | Replaced — `Start/Stop/CurrentStatus/Probe/RegisterHttpServer/RegisterStdioServer/DeregisterServer/ListTools/GatewayMcpUrl/AdapterType` all implemented. | PHASE-02 | done |
...
| `/api/clients/{id}/config` (ADR-001) | Unchanged. Per-LanClient maintainer-surface bundle download remains for the privilege-flag integration model. | Stays on maintainer surface; not the AI-client path. | — | keep |
| Browser onboarding UI (`resources/web/app.js`) | Existing maintainer dashboard; not yet pointed at `/api/onboarding/*`. | PHASE-09 (Tron dashboard realignment) wires the UI. Profiles are content-stable today and ready for that consumer. | PHASE-09 | pending |
...
| Surface | Today | Realignment target | Resolves in | Action |
|---|---|---|---|---|
| `resources/clu/governance-profile.json` | Unchanged. Source of truth for CLU doctrine, policies, action kinds, roles, rules. `GovernanceBundleService` reads this on every bundle request so maintainer edits propagate without restart. | Source of truth honored. | PHASE-05 | done |
| `GET /api/clu`, `GET /api/clu/tools`, `GET /api/clu/apple-operations` | Unchanged. Maintainer-facing CLU surface preserved. | Stays on the maintainer surface. | — | keep |
| `GET /api/client/governance/profile`, `POST /api/client/governance/decisions` | Unchanged. Per-LanClient governance read/decide path remains for maintainer-flagged clients. | Stays. The new AI-client governance path is the bundle URL embedded in the onboarding profile (PHASE-04 + PHASE-05). | — | keep |
...
| `/api/runtime/mcp-servers`, `/api/runtime/subagents`, `/api/runtime/subagent-groups` | Unchanged maintainer-facing catalog CRUD. | Stays on the maintainer surface as the way maintainers register backends; per-pool runtime state is added alongside via `/api/pools`. | PHASE-06 / PHASE-07 | extended |
...
| Surface | Today | Realignment target | Resolves in | Action |
|---|---|---|---|---|
| `resources/web/app.js`, `index.html`, `styles.css` | **Landed PHASE-09.** Eleven destinations: Overview (gateway-first KPIs), Gateway, Pools, Clients (presence), LAN Identity (maintainer), Governance, Onboarding, Discovery, Shared Fabric (maintainer catalog), Activity (telemetry events + legacy admin ring), Exports. Toolbar quick-actions: Onboard a Client, Gateway Status, Worker Pools, Discovery, Governance, LAN Identity. Header subtitle realigned to gateway-first wording. | Replaced. | PHASE-09 | done |
...
| Browser auth | None — trusted LAN per ADR-001 §3. | Maintainer surface stays as trusted LAN; AI-client surface moves to a logically distinct gateway port with `auth=none`/`trust=lan`. | PHASE-10 | split |
| WinUI shell (`src/MasterControlShell/`) | Builds clean (1 pre-existing C4100 warning). Not yet wired to gateway-first panels — PHASE-09 deliberately scoped to the browser surface per the phase file's `readFirst`. | Either delete the shell entirely or rebuild it around the new model in a follow-on track; ADR-001 already accepted browser-only maintainer surface during the rebuild. PHASE-09 leaves it untouched. | PHASE-10 / future | deferred |

## H. Trust / authentication boundary

| Surface | Today | Realignment target | Resolves in | Action |
|---|---|---|---|---|
| Maintainer surface (`/api/clients/*`, `/api/runtime/*`, `/api/forsetti/*`, `/api/clu/approvals`, `/api/setup/*`, `/api/exports`, `/api/install/*`) | LAN-trusted; `X-MCOS-Client-Id` middleware identifies actor for activity attribution; per-client privilege flags gate mutations. | **Unchanged**. ADR-002 §1 keeps this model on the maintainer surface. | — | keep |
| AI-client surface (today: `GET /api/client/mcp-servers`, `/api/client/sub-agents`, `/api/client/activity`, `/api/client/governance/profile`, `POST /api/client/governance/decisions`, `/api/client/heartbeat` at `MasterControlRuntime.cpp:8519–8598`) | Per-LanClient with required `X-MCOS-Client-Id`. | The new AI-client gateway URL replaces this as the primary tool path. The existing routes can stay as a per-client read-surface for clients that opt into the maintainer-style integration, but they are no longer the documented AI-client onboarding target. | PHASE-04, PHASE-09 | split |
...
| `.github/workflows/release.yml` | **Removed at A3.11.0.** It is not part of the current alpha release path. Alpha versions are published as GitHub pre-releases with MSI artifacts after the same-SHA `windows-build-test-package.yml` product gate passes; operator deployment Gates D/E remain separate evidence. | Removed from current path. | PHASE-10 | done |
| `VERSION.json` | **Bumped 0.5.0 → 0.6.0 in PHASE-10.** First allowed bump per `manifest.json` `noBumpUntilPhaseTen: true`. Strategy `minor-on-architecture-change` applies because the public AI-client surface fundamentally changed but ADR-001 maintainer surface is preserved. New release entry summarizing all PHASE-00..10 work. | Bumped. | PHASE-10 | done |
...
| Process execution 7-step rule | **Audited PHASE-10.** The synchronous executor at `MasterControlRuntime.cpp:914-1110` (`runHostedExecutable`) implements all seven steps cleanly. The supervised long-running paths (`WorkerSupervisor::startInstanceLocked`, `NativeHttpSysGatewayAdapter::Start`) use Job Object containment for tree-kill on shutdown. Two pre-existing bootstrapper sites use `INFINITE` waits without timeout/kill — flagged with `// PHASE-10 known-issue` comments and FORBIDDEN-CONTRACT §6.4 exemptions. | Documented compliance + exemptions. | PHASE-10 | done (audit) / deferred (bootstrapper rewrite) |
| Firewall / LAN mode docs | **Landed PHASE-10.** New `docs/wiki/Operations/Windows-Firewall-LAN-Mode.md` covers the trust model boundary, four `New-NetFirewallRule` snippets (gateway / maintainer / DNS-SD / beacon), Public profile blocks, validation snippets, and uninstall cleanup. | Replaced. | PHASE-10 | done |
...
| Bootstrapper smoke check | The product gate runs `MasterControlBootstrapper.exe preflight <staged-payload> --skip-service --skip-firewall --skip-uninstall-registration --json`, fails on missing bootstrapper/nonzero exit/invalid JSON/`ready=false`, and retains the JSON in the package artifact. | Honored. | PHASE-10 | done |

## J. Native gateway evaluation

| Surface | Today | Realignment target | Resolves in | Action |
|---|---|---|---|---|
| the in-process HTTP.sys adapter vs the in-process HTTP.sys adapter | **Decided PHASE-11 (ADR-003).** Keep the external supervised substrate as the v0.6.x default substrate behind the existing `IMcpGateway` adapter. The `IMcpGateway` abstraction makes the choice reversible: a a native HTTP.sys substrate is documented as a conditional future phase (proposed PHASE-12) gated on five operational triggers (measured throughput cap, session-affinity break, upstream stall, maintainer-install friction, vendoring policy change). Full evaluation lives in `docs/implementation/PHASE-11-NATIVE-GATEWAY-EVALUATION.md`. | Decision recorded; native rebuild deferred until evidence justifies it. | PHASE-11 | done (decision) / deferred (native rebuild) |
| Native gateway requirements documented | **Landed PHASE-11.** `docs/implementation/PHASE-11-NATIVE-GATEWAY-EVALUATION.md` §2 enumerates surface compatibility, required protocol features (Streamable HTTP, tools registry by logical server name, stdio bridge, session affinity, auth=none/trust=lan), Windows-native fit (HTTP.sys + ETW + SCM integration), telemetry fit, test fit, and a 7-18 week effort estimate. | Documented. | PHASE-11 | done |
| Migration plan for substrate swap | **Landed PHASE-11.** Memo §3 covers pre-flight (interface stability check, contract audit), implement-side-by-side (peer adapter, no the in-process HTTP.sys adapter deletion), test-side-by-side (re-run PHASE-02 tests + new HTTP.sys-specific tests + soak comparison), maintainer rollout (default flip after at least one release-gate cycle), post-migration validation (FORBIDDEN-CONTRACT §2.1 auto-extends, no Forsetti touch, version bump per minor-on-architecture-change). | Documented. | PHASE-11 | done |
| external-substrate operational limitations report | **Landed PHASE-11.** Memo §4 enumerates limitations confirmed today (separate maintainer install, process-boundary cost, opaque internal state, upstream cadence dependency, coarse crash recovery, Go runtime overhead) and limitations suspected but not measured (throughput ceiling, connection-affinity edge cases, multi-day memory growth). | Documented. | PHASE-11 | done |
...
- The browser-as-primary-maintainer-surface decision (ADR-001 negative-consequences acknowledgement).
