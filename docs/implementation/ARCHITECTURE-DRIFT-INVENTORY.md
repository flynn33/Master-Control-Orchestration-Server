...
## B. MCP Gateway surface

| Surface | Today | Realignment target | Resolves in | Action |
|---|---|---|---|---|
| MCP Gateway adapter | None. No `IMcpGateway`, no `McpJungleGatewayAdapter`, no `mcpgateway`/`mcpjungle` symbols anywhere in `src/`. | New C++ interface `IMcpGateway` with `Start/Stop/Probe/RegisterHttpServer/RegisterStdioServer/DeregisterServer/ListTools/GatewayMcpUrl`. First implementation `McpJungleGatewayAdapter` supervises MCPJungle as a child process. | PHASE-02 | replace (new) |
| Gateway configuration | Not present. | `gateway-service.schema.json`-conformant configuration with `gatewayType ∈ {mcpjungle, native}`, `enabled`, `binaryPath`, `listenHost/Port`, `mcpPath`, `healthPath`, `databasePath`, `mode ∈ {development, enterprise, lan-trusted}`. | PHASE-02 | replace (new) |
| Gateway health endpoint | `GET /api/health` exists for runtime health (`MasterControlRuntime.cpp:8474`); no gateway-specific health. | Surface gateway health through the runtime API; populate the dashboard's gateway panel. | PHASE-02, PHASE-09 | extend |
| Logical pool registration with gateway | Not present. | Each `ManagedEndpointPool` registers a stable logical MCP server with the gateway adapter; autoscaled instances are NOT registered separately. | PHASE-02 (path), PHASE-06 (pools) | replace (new) |
