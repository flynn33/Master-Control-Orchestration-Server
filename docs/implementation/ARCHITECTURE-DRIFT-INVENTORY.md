...
| Surface | Today | Realignment target | Resolves in | Action |
|---|---|---|---|---|
| MCPJungle vs native HTTP.sys gateway | **Decided PHASE-11 (ADR-003).** Keep MCPJungle as the v0.6.x default substrate behind the existing `IMcpGateway` adapter. The `IMcpGateway` abstraction makes the choice reversible: a native HTTP.sys gateway is documented as a conditional future phase (proposed PHASE-12) gated on five operational triggers (measured throughput cap, session-affinity break, upstream stall, operator-install friction, vendoring policy change). Full evaluation lives in `docs/implementation/PHASE-11-NATIVE-GATEWAY-EVALUATION.md`. | Decision recorded; native rebuild deferred until evidence justifies it. | PHASE-11 | done (decision) / deferred (native rebuild) |
...
| Migration plan for substrate swap | **Landed PHASE-11.** Memo §3 covers pre-flight (interface stability check, contract audit), implement-side-by-side (peer adapter, no MCPJungle deletion), test-side-by-side (re-run PHASE-02 tests + new HTTP.sys-specific tests + soak comparison), operator rollout (default flip after at least one release-gate cycle), post-migration validation (FORBIDDEN-CONTRACT §2.1 auto-extends, no Forsetti touch, version bump per minor-on-architecture-change). | Documented. | PHASE-11 | done |
| MCPJungle operational limitations report | **Landed PHASE-11.** Memo §4 enumerates limitations confirmed today (separate operator install, process-boundary cost, opaque internal state, upstream cadence dependency, coarse crash recovery, Go runtime overhead) and limitations suspected but not measured (throughput ceiling, connection-affinity edge cases, multi-day memory growth). | Documented. | PHASE-11 | done |
