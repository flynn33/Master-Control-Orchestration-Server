...
If every line prints a `leaseId` and a corresponding `instanceId`, the supervisor + lease router stack is healthy end-to-end. AI clients consuming the gateway path (`http://<host>:8080/mcp/...` or `/agents/...`) inherit the same routing once native HTTP.sys gateway (or any future native gateway) is bound to TCP 8080 — see [Packaging and Gateway Binary](Packaging-and-Gateway-Binary) for the gateway-binary side.
