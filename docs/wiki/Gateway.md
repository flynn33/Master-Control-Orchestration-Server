...
- **Backpressure:** the gateway executes MCP requests on a bounded worker pool; `/health` is always answered inline and never queues behind slow `tools/call` traffic, and request bodies are drained on the workers so a slow-uploading client cannot stall intake. Request bodies are capped at 10 MB (`413 Payload Too Large` beyond that). When the request queue is saturated the gateway answers a structured `503` with retry guidance.
