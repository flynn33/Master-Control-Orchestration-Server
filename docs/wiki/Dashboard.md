...
### Gateway
Adapter type (`mcpjungle`), lifecycle state (`running` / `configured` / `failed`), health (`healthy` / `degraded` / `unhealthy` / `unknown`), the live advertised MCP URL, and the registered tools list. Reads from `/api/gateway/{status,health,tools}`.
