...
- v0.9.0 status update date: 2026-05-07 (native HTTP.sys gateway retired; native HTTP.sys is the only shipping substrate)
...
### Status update (v0.9.0) — native HTTP.sys gateway retired
...
The original ADR-003 decision (recorded below for the historical record) was to **keep native HTTP.sys gateway as the v0.6.x substrate** and defer a native HTTP.sys gateway to a conditional PHASE-12 gated on five operational triggers. That decision held through v0.6.0 through v0.6.8.
...
- "Keep native HTTP.sys gateway for v0.6.x" → "Both substrates ship; operators select." The ADR-002 §2 reversibility guarantee is satisfied in both directions.
...
- Default for fresh installs as of v0.7.0 is encouraged to be `mcpGateway.type = "native"` because there is no separate binary to install. Existing v0.6.x deployments stay on `native HTTP.sys gateway` unless they choose to switch.
...
ADR-002 §2 chose native HTTP.sys gateway as the first MCP gateway substrate behind a replaceable `IMcpGateway` adapter. ADR-002 §11 also forbade vendoring third-party code into MCOS, so native HTTP.sys gateway was integrated as a supervised external child process (PHASE-02), never as in-tree source. The realignment manifest reserved PHASE-11 for the question this ADR answers: **after the spike (PHASE-02) and the operational layers built on top of it (PHASE-03 through PHASE-10), should MCOS keep native HTTP.sys gateway, or replace it with a native HTTP.sys-backed gateway implemented inside MCOS?**
...
2. **Operational evidence we would need to justify a native rebuild does not exist.** The dev environment never spawned a real gateway binary (the supervised-mock fallback handled its absence honestly). We have architectural review, not measurements. The honest-evidence rule from ADR-002 §10 ("Do not claim runtime behavior unless it was tested or directly proven") cuts both ways — it forbids claiming native HTTP.sys gateway is good enough *and* forbids claiming the native rebuild is necessary. What we have is a small, well-pinned adapter layer and an outstanding question.
...
**Keep native HTTP.sys gateway as the v0.6.x default gateway substrate.** The `IMcpGateway` adapter shape is preserved, no source code changes, no version bump, no client contract changes.
...
1. **Measured throughput cap with operational impact.** Real LAN AI-client traffic shows native HTTP.sys gateway saturating below the p95 we need to support.
2. **Confirmed session-affinity break under load.** PHASE-07's sticky-session contract (FORBIDDEN-CONTRACT §2.4) is observably violated by native HTTP.sys gateway's routing.
3. **native HTTP.sys gateway release-cadence stall.** Upstream does not track an MCP spec version we need to ship.
4. **Operator-experience friction blocks adoption.** New operators consistently fail to install native HTTP.sys gateway alongside MCOS.
...
- ADR-002 §11 remains in force. Forsetti is sealed; native HTTP.sys gateway is supervised, not vendored. Any future native gateway is first-party MCOS C++ that respects the same vendoring rule.
...
- The Windows-native posture of the product is partially compromised. native HTTP.sys gateway is a Go binary supervised by Job Object — Windows-compatible but not Windows-native. The marketing claim "Windows-native" is true of MCOS itself; it is *not* true of the gateway substrate today.
- Operator first-time-install friction persists. native HTTP.sys gateway is a separate download and configure step.
...
- Either outcome of a future PHASE-12 (keep-native HTTP.sys gateway or build-native) stays inside the `IMcpGateway` adapter. ADR-002 §2's reversibility guarantee holds in both directions.
...
- [Gateway](Gateway) — substrate selection, native HTTP.sys gateway install steps, native HTTP.sys enablement, stdio bridge behavior
