...
- **Critical performance fix: admin API mutating handlers were blocking for 10-14 seconds on synchronous endpoint probes.** `RuntimeInventoryService::refresh()` iterates every seeded endpoint (35+ in a populated install) and calls `probeEndpoint()` for each — a blocking TCP connect with a 400 ms timeout. When that loop ran synchronously from inside `upsertMcpServer`, `removeMcpServer`, `upsertSubAgent`, `removeSubAgent`, and `upsertProvider`, every mutating admin API call blocked for ~14 seconds while the probe swept the inventory. A feature-test harness run against a live service reproduced 10-second HTTP timeouts on all three endpoints (`POST /api/runtime/mcp-servers`, `POST /api/runtime/mcp-servers/remove`, `POST /api/runtime/subagents`). Added `IRuntimeInventoryService::refreshAsync()` — a non-blocking variant that fires the probe loop on a detached thread with a pending-flag guard so stacked callers coalesce. Switched all 5 mutating call sites to `refreshAsync()`. After the fix, the same endpoints return in **2-18 ms** instead of 10000 ms. Synchronous `refresh()` is still used by the snapshot-capture path where the caller needs fresh probe state. (`include/MasterControl/MasterControlContracts.h`, `src/MasterControlApp/MasterControlRuntime.cpp`)
- **Race fix: `SubAgentGroupService::upsertGroup` validated member target ids against the stale inventory cache.** After the async refresh fix, a sub-agent upserted 17 ms earlier was not yet visible in `inventoryService_->listEndpoints()`, so a group creation that referenced it failed with HTTP 400 "Select at least one valid sub-agent for the group." Switched the validator to read `state_->configuration.activeProfile.seededEndpoints` directly under the state mutex — the authoritative source of truth, updated synchronously by the sub-agent upsert handler. (`src/MasterControlApp/MasterControlRuntime.cpp`)
- Validated end-to-end with an autonomous `feature-test.py` harness that hits every admin API route (smoke / telemetry / forsetti modules / provider capabilities / auto-connect × 4 kinds / config persistence / assignment targets / MCP server upsert+remove / sub-agent upsert / sub-agent group upsert / CLU governance profile / exports list / activity ring) and produces a structured JSON-L stream plus a human-readable markdown report. Final run on Windows Server 2022: **22 PASS / 0 FAIL / 0 WARN**, all single-digit-millisecond latencies. Harness saved at `.claude-work/feature-test.py` for future repair cycles.
